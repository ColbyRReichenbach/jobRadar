...
async def _track_contact_response(db, sender_email: str):
    ...
    stmt = select(Contact).where(Contact.email == clean_email)
...
async def _get_feedback_blocklist(db) -> set[str]:
    """Get sender domains that the user has marked as not job-related."""
    from backend.models import EmailFeedback

    stmt = select(EmailFeedback.sender_domain).where(
...
async def _poll_gmail_async():
    ...
    async with async_session_factory() as db:
        ...
        # Fetch all recent emails — let the LLM classifier decide relevance
        results = (
            service.users()
            .messages()
        ...
        if not messages:
            logger.info("No new Gmail messages found")
            return 0

        company_domains = await get_active_company_domains(db)
        feedback_blocklist = await _get_feedback_blocklist(db)
        processed = 0
        skipped_feedback = 0
        ...
        for msg_ref in messages:
            ...
            if await email_already_processed(db, msg_id):
                continue
            ...
            # Skip domains user has explicitly marked as not job-related
            ...
            # Parse body using new multipart MIME parser
            body = parse_email_body(full_msg.get("payload", {}))

            # Track contact responses
            await _track_contact_response(db, sender_email)
            ...
            # Skip emails classified as not_relevant
            ...
            # Use LLM-extracted company name if available, else domain-based
            email_company_name = (
            ...
            application_id = await match_email_to_application(
                db, email_dict, classification
            )

            # Build the email event with enriched data
            ...
            # Update application status if warranted
            if cls in STATUS_UPDATES and application_id:
                await update_application_status(db, application_id, classification)

            processed += 1

        await db.commit()
        logger.info(f"Processed {processed} emails, skipped {skipped_feedback} from feedback blocklist")
        return processed
