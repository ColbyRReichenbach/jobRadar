...
function EmailListCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-base font-serif font-bold text-slate-900 truncate">
              {email.sender}
            </div>
      ...
      <p className="text-xs line-clamp-2 leading-relaxed text-slate-500">
        {email.snippet}
      </p>

      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100/70">
        <div
          className={cn(
        ...
        <button
          onClick={(event) => onNotJobRelated(event, email)}
...
export function EmailFeed({
    ...
    () =>
      emails.filter((email) => {
        ...
        if (
          ...
          email.classification === 'action_item'
        ) {
          return true;
        }
        return !isNoise;
      }),
    [dismissedEmails, emails],
  );
  ...
  useEffect(() => {
    if (selectedEmail && !feedEmails.some((email) => email.id === selectedEmail.id)) {
      setSelectedEmail(null);
      setPipelineAlert(null);
    }
  }, [feedEmails, selectedEmail]);

  useEffect(() => {
    if (!focusRequest) return;
    setSearchQuery('');
    setFilter('all');
    const target = emails.find((email) => email.id === focusRequest.emailId && email.type !== 'conversation');
    if (target) {
    ...
    <div className={cn('flex h-full bg-[#F5F5F0]', forceOpen ? 'w-full' : '')}>
      <motion.div
        ...
        className={cn(
          ...
          forceOpen
            ? selectedEmail
              ? 'w-full md:w-[400px] lg:w-[450px] hidden md:flex'
      ...
      >
        ...
        <div
          className={cn(
            ...
            collapsed ? 'flex justify-center px-0 items-center' : forceOpen ? 'block' : 'flex items-center justify-between',
          )}
        >
          {forceOpen ? (
            <>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full font-semibold">
                  {feedEmails.filter((email) => !email.read).length} new
                </span>
              </div>
            ...
            <>
              <div className="flex items-center gap-2">
                ...
                {!collapsed && (
                  <h2 className="text-3xl tracking-tight font-serif font-bold text-slate-900">
                    Updates
                  </h2>
                )}
              </div>
          ...
          {!forceOpen && selectedEmail ? (
            ...
            >
                ...
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-100">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-slate-400 truncate">
                      {selectedEmail.senderEmail || 'unknown'} ·{' '}
              ...
              <div className="p-3 border-t border-slate-100 flex gap-2">
                ...
                <button
                  onClick={(event) => handleNotJobRelated(event, selectedEmail)}
                ...
                <div className="h-full overflow-y-auto p-4 space-y-2">
                  {feedEmails.map((email) => (
                    ...
                    >
                      {getClassificationIcon(email.classification)}
                      {!email.read && (
                        <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-indigo-500 rounded-full border-2 border-white" />
                      )}
                    </div>
                  ))}
                </div>
                  ...
                  {filteredInboxEmails.length === 0 ? (
                    ...
                    filteredInboxEmails.map((email) => (
                      <EmailListCard
                        ...
                        getEmailCompany={getEmailCompany}
                      />
                    ))
                  )}
                </div>
        ...
        <div className={cn('flex-1 flex flex-col bg-white h-full overflow-hidden', !selectedEmail && 'hidden md:flex')}>
          {selectedEmail ? (
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-2xl tracking-tight font-serif font-bold text-slate-900">
                        {selectedEmail.subject}
                      </h2>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-[#F5F5F0]/50">
              ...
              <h3 className="text-2xl tracking-tight font-serif font-bold text-slate-900 mb-2">
                No Update Selected
              </h3>
              <p className="text-slate-500 max-w-sm">
                Select an inbox update from the list to view the full details.
              </p>
            </div>
          )}
        </div>
