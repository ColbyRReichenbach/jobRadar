...
Status note: this is the original planning spec. Later implementation added `ActionCandidate`, shared dedupe services, `UserKnowledgeDocument`, `DocumentChunk`, and `RetrievalTrace`. Treat the static counts below as superseded unless regenerated locally with `scripts/collect_runtime_counts_artifact.py`.

### Local Runtime Counts

The original draft included a hand-copied local database snapshot. That table is no longer treated as product evidence. Runtime counts must be generated by a script that records database label, migration version, git SHA, timestamp, table counts, and missing-table warnings.
...
- `ActionCandidate` and shared dedupe services now exist for several AI/Gmail/calendar/Radar action-producing paths.
- Direct user CRUD flows are still normal product writes, not uniformly represented as action candidates.
- The same job can still enter from extension, Gmail, Radar, or Copilot unless each path consistently calls shared dedupe.
...
- No BM25 ranking.
- Chunk-level lexical retrieval, traces, eval gates, and shadow tracing exist, but they are not promoted as the default product retrieval path.
- No reranking.
- No query rewriting or intent-aware retrieval.
- No source trust/freshness scoring.
- Retrieval eval gates exist for local comparison, but product-facing retrieval promotion still depends on real labeled outcomes.
...
2. Use the existing `UserKnowledgeDocument`, `DocumentChunk`, chunking, `RetrievalTrace`, and lexical chunk eval foundation.
3. Promote chunk retrieval only after real-label evals justify the product routing change.
4. Add embeddings for chunks.
5. Add BM25 or OpenSearch lexical scoring.
6. Add hybrid merge.
7. Add reranking.
8. Add retrieval evals before using retrieval for high-risk generation.
